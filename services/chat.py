...
from schemas.chat import DialogIn, Message, Dialog
...
class ChatService(AppService):
    async def handle_chat(self, websocket: WebSocket, dialog_id: int, sender: Client, receiver_id: int):
        if not ChatCRUD(self.db).is_user_in_dialog(dialog_id, sender.id) or \
                not ChatCRUD(self.db).is_user_in_dialog(dialog_id, receiver_id):
