...
async def upload_file(data: dict, field_name: str, table_name: str, entity_id: int, multiple: bool):
    files = []
    if type(data.get(field_name)) == tuple or type(data.get(field_name)) == UploadFile or \
            type(data.get(field_name)) == list:
            ...
            if file.filename != '':
                if os.path.exists(f'media/files/{file.filename}'):
                    ...
                    with open(f'media/files/{file.filename}', 'wb') as f:
                        ...
                        f.write(content)
                        file_link = f.name[6:]
                new_files.append((file_link, False) if type(data[field_name]) == tuple else file_link)
            else:
                if entity_id:
                    db = next(get_db())
                    entity = db.query(tables_dict[table_name]).filter(tables_dict[table_name].id == entity_id).first()
