...
class IndexCRUD(AppCRUD):
    ...
    def like_article_by_id(self, id: int, user_id: int) -> Union[str, Exception]:
        ...
        if not article:
            return AppException.NotFoundException('Статья не найдена!')
        if not self.db.query(ArticleLike).filter(ArticleLike.article_id == article.id,
                                                 ArticleLike.client_id == user_id).first():
            article.likes += 1
    ...
    def dislike_article_by_id(self, id: int, user_id: int) -> Union[str, Exception]:
        ...
        if not article:
            return AppException.NotFoundException('Статья не найдена!')
        article_like = self.db.query(ArticleLike).filter(ArticleLike.article_id == id,
                                                         ArticleLike.client_id == user_id).first()
        if article_like:
            article.likes -= 1
    ...
    def create_article_comment(self, article_id: int, data: ArticleCommentIn, client_id: int) -> ArticleComment:
        comment = ArticleComment(article_id=article_id, sender_id=client_id, text=data.text)
        self.db.add(comment)
        self.db.commit()
    ...
    def like_comment_by_id(self, comment_id: int, client_id: int) -> Union[dict, Exception]:
        ...
        if comment_like:
            return AppException.ForbiddenException('Вы уже поставили лайк на этот комментарий!')
        comment = self.db.query(ArticleComment).filter(ArticleComment.id == comment_id).first()
        if not comment:
            return AppException.NotFoundException('Комментарий не найден!')
        comment.likes += 1
        new_comment_like = ArticleCommentLike(comment_id=comment_id, client_id=client_id)
    ...
    def dislike_comment_by_id(self, comment_id: int, client_id: int) -> Union[dict, Exception]:
        comment_like = self.db.query(ArticleCommentLike).filter(ArticleCommentLike.comment_id == comment_id,
                                                                ArticleCommentLike.client_id == client_id).first()
        if not comment_like:
            return AppException.NotFoundException('Вы не ставили лайк этому комментарию!')
        comment = self.db.query(ArticleComment).filter(ArticleComment.id == comment_id).first()
        comment.likes -= 1
