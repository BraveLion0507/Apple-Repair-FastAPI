...
class ChatCRUD(AppCRUD):
            ...
            for file in files:
                if type(file) == dict:
                    ...
                    if os.path.exists(f'media/files/{filename}'):
                        i = 1
                        paths_parts = [filename[:filename.rindex('.')], filename[filename.rindex('.') + 1:]]
                        print(paths_parts)
    ...
    def get_dialogs_by_user_id(self, user_id: int) -> List[dict]:
        dialogs = self.db.query(Dialog).filter(or_(Dialog.sender1_id == user_id, Dialog.sender2_id == user_id)).all()
        return list(dialogs)

    def get_messages_by_dialog_id(self, dialog_id: int) -> List[Message]:
        messages = self.db.query(Message).filter(Message.dialog_id == dialog_id).order_by(Message.sent_at).all()
    ...
    def make_read(self, messages: List[int], sender: int) -> List[Message]:
        ...
        if len(db_messages) > 0:
            unread_message = self.db.query(UnreadMessage).filter(
                UnreadMessage.client_id == sender, UnreadMessage.dialog_id == db_messages[0].dialog_id).first()
            if not unread_message:
                unread_message = UnreadMessage(client_id=sender, dialog_id=db_messages[0].dialog_id)
                self.db.add(unread_message)
                self.db.commit()
        for message in db_messages:
            if (sender == message.dialog.sender1_id and message.dialog.sender1_id != message.sender_id) or \
                    (sender == message.dialog.sender2_id and message.dialog.sender2_id != message.sender_id):
                message.is_read = True
                new_messages.append(message)
            else:
                raise AppException.ForbiddenException('Вы не можете просматривать это сообщение!')
    ...
    def get_unread_messages_by_user_id(self, client_id: int) -> List[UnreadMessage]:
        unread_messages = self.db.query(UnreadMessage).filter(UnreadMessage.client_id == client_id).all()
