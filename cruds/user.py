...
from schemas.user import ClientRegister, MasterRegister, MasterIn, UnreadMessageOut, RecoverPasswordIn, ChangePasswordIn
...
class UserCRUD(AppCRUD):
    async def create_client(self, client: ClientRegister) -> Union[dict, Exception]:
        ...
        if self.db.query(Client).filter(Client.email == client.email).first() is not None:
            return AppException.AlreadyExistsException(detail='Пользователь с такой почтой уже существует!')

        validated_phone = phone_validator(client.phone)
        if not validated_phone:
            return AppException.ValidationException('Некорректный номер телефона!')
        if self.db.query(Client).filter(Client.phone == str(client.phone)).first() is not None:
            return AppException.AlreadyExistsException(detail='Пользователь с таким номером телефона уже существует!')
        ...
        user = Client(name=client.name, lastname=client.lastname, email=client.email, phone=validated_phone,
    ...
    def get_jwt_tokens(self, data: OAuth2PasswordRequestForm) -> Union[dict, Exception]:
        ...
        refresh_token_check = self.db.query(RefreshToken).filter(RefreshToken.user_id == user.id)
        if refresh_token_check.first():
            if refresh_token_check.first().date_created + datetime.timedelta(
                    minutes=REFRESH_TOKEN_EXPIRE_MINUTES) < datetime.datetime.now():
    ...
    async def create_master(self, id: int, master: Union[MasterRegister, MasterIn]) -> Union[dict, Exception]:
        ...
        for repair in repair_types:
            master_repair = self.db.query(MasterRepair).filter(MasterRepair.master_id == user.username,
                                                               MasterRepair.repair_id == repair.id).first()
            if not master_repair:
                new_repair = MasterRepair(master_id=user.username, repair_id=repair.id,
                                          address_latitude=master.address_latitude,
                                          address_longitude=master.address_longitude)
    ...
    def update_client_by_id(self, id: int, data: dict, file: UploadFile) -> Union[Client, Exception]:
        ...
        if 'phone' in data and data['phone'] != client.phone:
            is_client = self.db.query(Client).filter(Client.phone == data['phone']).first()
            if is_client and is_client.id != id:
                return AppException.AlreadyExistsException('Такой номер телефона уже используется!')
            client.is_phone_verified = False
        if 'email' in data and data['email'] != client.email:
            is_client = self.db.query(Client).filter(Client.email == data['email']).first()
            if is_client and is_client.id != id:
                return AppException.AlreadyExistsException('Такая почта уже используется!')
        ...
        if 'new_password1' in data:
            ...
            is_valid_password = password_validator(data['new_password1'], data['new_password2'])
            if not is_valid_password:
                return AppException.ValidationException(detail='Некорректный пароль!')
        if file:
            if os.path.exists(f'media/files/{file.filename}'):
                i = 1
                paths_parts = [file.filename[:file.filename.rindex('.')], file.filename[file.filename.rindex('.') + 1:]]
    ...
    def get_master_by_username(self, username: str) -> Union[dict, Exception]:
        ...
        new_master.update({'name': master.client.name,
    ...
    def update_master_by_username(self, username: str, data: dict, pictures: List[UploadFile]) -> Union[dict, Exception]:
        ...
        if data.get('address_latitude'):
            master_repairs = self.db.query(MasterRepair).filter(MasterRepair.master_id == username).all()
    ...
    def get_master_by_client(self, user: Client) -> Master:
        ...
        if not master:
            raise HTTPException(
                status_code=404,
                detail='Не удалось найти мастера!'
            )
        return master
    ...
    def notification_handler(self, data: dict, user: models.user.Client):
        ...
        try:
            ...
            if notification_type == 1:
                ...
                if not (data['receiver_id'], data['dialog_id']) in manager.active_connections:
                        receiver = data['receiver_id']
                        unread_message = self.db.query(UnreadMessage).filter(
                            UnreadMessage.client_id == data['receiver_id'],
                            UnreadMessage.dialog_id == data['dialog_id']).first()
                        if not unread_message:
                            unread_message = UnreadMessage(client_id=data['receiver_id'],
                            ...
                            if data['message_id'] not in unread_message.messages:
                                ...
                                self.db.commit()
                                self.db.refresh(unread_message)
                        messages = self.db.query(UnreadMessage).filter(
                            UnreadMessage.client_id == data['receiver_id']).all()
                        unread_messages = list()
                        for message in messages:
                            unread_messages.append(UnreadMessageOut(**message.__dict__).model_dump(mode='json'))
                        new_data = {
                            'type': 2,
                            'sender': user.id,
                ...
                receiver = data['receiver_id']
                new_user = self.db.query(Client).filter(Client.id == receiver).first()
                if not new_user:
                    raise AppException.NotFoundException('Получатель не найден!')
                notification = Notification(type=NotificationTypeEnum.accepted_order, receiver_id=receiver,
                ...
                if not new_user:
                    raise AppException.NotFoundException('Получатель не найден!')
                notification = Notification(type=NotificationTypeEnum.new_offer, receiver_id=receiver,
                ...
                if not new_user:
                    raise AppException.NotFoundException('Получатель не найден!')
                notification = Notification(type=NotificationTypeEnum.accepted_offer, receiver_id=receiver,
    ...
    def write_commission_off(self, master: Master, price: float) -> Master:
        ...
        if master.balance < amount:
            raise AppException.PaymentRequiredException('Недостаточно средств, пополните счет!')
    ...
    def replenish_balance(self, master: Master, amount: float) -> Union[dict, Exception]:
        ...
        new_payment = DBPayment(master_username=master.username, id=return_id, amount=amount)
    ...
    def confirm_payment(self, return_id: uuid.UUID) -> Union[dict, Exception]:
        ...
        if kassa_payment.status == 'succeeded':
            master = self.db.query(Master).filter(Master.username == payment.master_username).first()
    ...
    def get_unread_messages(self, user_id: int) -> List[UnreadMessage]:
        unread_messages = self.db.query(UnreadMessage).filter(UnreadMessage.client_id == user_id).all()
        return list(unread_messages)

    async def send_mailing(self, master_username: str, order_id: int = None, request_id: int = None) -> None:
        if master_username == '__all__':
            masters = self.db.query(Master).filter(Master.mailing == True, Master.is_active == True).all()
            for master in masters:
                if order_id:
                    email = Email(master.client, email=[master.client.email], entity_id=order_id, type=2)
                    await email.send_mailing('tsarbirzzha.ru | Уведомление!')
                elif request_id:
                    email = Email(master.client, email=[master.client.email], entity_id=request_id, type=3)
                    await email.send_mailing('tsarbirzzha.ru | Уведомление!')
            return

        master = self.db.query(Master).filter(Master.username == master_username).first()
    ...
    def get_deposit_history_by_master(self, master: Master) -> List[DBPayment]:
        payments = self.db.query(DBPayment).filter(
    ...
    async def recover_password(self, data: RecoverPasswordIn) -> Union[dict, Exception]:
        ...
        await Email(user, [user.email], code).send_password_recovery_code('Восстановление пароля TsarBirzzha')
    ...
    def change_password(self, data: ChangePasswordIn) -> Union[dict, Exception]:
        ...
        if not password_validator(data.password):
            return AppException.ValidationException('Пароль не удовлетворяет требованиям!')
