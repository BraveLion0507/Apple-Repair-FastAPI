...
class ServiceCRUD(AppCRUD):
    ...
    def get_category(self, id: int) -> ServiceCategory:
        category = self.db.query(ServiceCategory).filter(ServiceCategory.id == id).first()
        return category
    ...
    def update_repair_type(self, id: int, data: RepairTypeEdit, master: Master) -> Union[RepairType, Exception]:
        ...
        self.update_master_repair(repair_type.id, MasterRepairEdit(**data.model_dump(exclude_unset=True)), master)
        return repair_type

    def update_master_repair(self, repair_id: int, data: MasterRepairEdit, master: Master) -> MasterRepair:
        ...
        if not master_repair:
            new_master_repair = MasterRepair(master_id=master.username, repair_id=repair_id, price=data.price,
                                             time=data.time, address_longitude=master.address_longitude,
                                             address_latitude=master.address_latitude)
            self.db.add(new_master_repair)
            self.db.commit()
    ...
    def get_all_master_repairs(self) -> List[MasterRepair]:
        ...
        for master_repair in master_repairs:
            if master_repair.master_id in master_usernames:
                continue
            master = self.db.query(Master).filter(Master.username == master_repair.master_id).first()
            if not master:
                continue
    ...
    def get_all_master_services(self, username: str) -> dict:
        ...
        for master_repair in master_repairs:
            if master_repair in repairs:
                continue
            repair = self.db.query(RepairType).filter(RepairType.id == master_repair.repair_id).first()
            ...
            if device.service_id in services:
                continue
            service = self.db.query(ServiceType).filter(ServiceType.id == device.service_id).first()
    ...
    def delete_master_repair_by_repair_id(self, repair_id: int, master: Master) -> Union[dict, Exception] :
        master_repair = self.db.query(MasterRepair).filter(MasterRepair.repair_id == repair_id,
                                                           MasterRepair.master_id == master.username).first()
        if not master_repair:
            return AppException.NotFoundException('Связь между мастером и видом ремонта не найдена!')
